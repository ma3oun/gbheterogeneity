...
def remove_version(tcga_genes: pd.DataFrame) -> pd.DataFrame:
    tcga_genes["Ensembl_only"] = tcga_genes.apply(lambda row: remove_ensembl_version_df(row), axis=1)

    return tcga_genes

def get_repeated_genes(tcga_genes: pd.DataFrame, column: str) -> pd.DataFrame:
    genes_without_version = tcga_genes[column]
    repeated_genes = tcga_genes[genes_without_version.isin(genes_without_version[genes_without_version.duplicated()])]
...
def get_mappings(filtered_genes_per_cluster: Dict, ensembl_to_tcga: Dict, ensembl_to_oncopole: Dict) -> Tuple:
...
def build_mappings(tcga_rna_path: str, oncopole_rna_path: str, mapping_file: str) -> Tuple:
    ...
    ensembl_list, ensembl_to_tcga, ensembl_to_oncopole = get_notation_mappings(common_genes)
    filtered_genes_per_chromosome = filter_genes_per_cluster(genes_per_cluster, ensembl_list)
    mapping_ensembl_to_tcga, mapping_ensembl_to_oncopole = get_mappings(filtered_genes_per_chromosome, 
                                                                          ensembl_to_tcga, 
                                                                          ensembl_to_oncopole)
