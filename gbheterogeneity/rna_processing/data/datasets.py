...
class RNASeq(data.Dataset):
    def __init__(self, tcga_split: str, gene_cluster_mapping_file: str, oncopole_rna_path: str) -> None:
    ...
    def get_genes_per_cluster(self):
        mapping_ensembl_to_public, _ = build_mappings(self.gene_files[0],
...
class LabeledRNASeq(RNASeq):
    def __init__(self, tcga_split: str, gene_cluster_mapping_file: str, oncopole_rna_path: str, site_mapping_path: str) -> None:
        self.tcga_split = tcga_split
        ...
        super().__init__(tcga_split=tcga_split,
...
class RNASeqOncopole(data.Dataset):
    ...
    def get_genes_per_cluster(self):
        mapping_ensembl_to_public, _ = build_mappings(self.public_gene_files[0],
                                                                  self.oncopole_rna_path,
                                                                  self.gene_cluster_mapping_file)
        return mapping_ensembl_to_public
...
def get_rna_attention_data(dataset_params: Dict, dataloader_params: Dict, num_gpus: int, global_rank: int, train: bool) -> data.DataLoader:
    dataset = RNASeq(tcga_split=dataset_params["tcga_split"], 
    ...
    if train and num_gpus:
        sampler = torch.utils.data.DistributedSampler(dataset, num_replicas=num_gpus, rank=global_rank, shuffle=True)
        loader = torch.utils.data.DataLoader(dataset, **dataloader_params, shuffle=False, pin_memory=True, sampler=sampler)
    elif train and num_gpus == 0:
        loader = torch.utils.data.DataLoader(dataset, **dataloader_params, shuffle=True)
    else:
        loader = torch.utils.data.DataLoader(dataset, **dataloader_params, shuffle=False)
    loader.genes_per_cluster = genes_per_cluster
    return loader

def get_rna_attention_labeled_data(dataset_params: Dict, dataloader_params: Dict, num_gpus: int, global_rank: int, train: bool) -> data.DataLoader:
    dataset = LabeledRNASeq(tcga_split=dataset_params["tcga_split"], 
                                     gene_cluster_mapping_file=dataset_params["gene_cluster_mapping_file"], 
                                     oncopole_rna_path=dataset_params["oncopole_rna_path"],
...
def get_rna_attention_data_oncopole(dataset_params: Dict, dataloader_params: Dict, num_gpus: int, global_rank: int, train: bool) -> data.DataLoader:
    dataset = RNASeqOncopole(oncopole_samples_directory=dataset_params["oncopole_samples_directory"], 
