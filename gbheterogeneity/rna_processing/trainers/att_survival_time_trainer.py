...
class AttentionTrainerSurvivalTime(simple_trainer.SimpleDistributedTrainer):
    ...
    def __init__(self,
        ...
        self.accuracy_3_months_key = "accuracy_3_months"
        self.accuracy_6_months_key = "accuracy_6_months"
        self.accuracy_12_months_key = "accuracy_12_months"        
    ...
    def compute_loss(self, survival_time: torch.Tensor, predictions: torch.Tensor) -> Dict:
        ...
        accuracy_3_months = ((survival_time - predictions) < 0.25).sum() / len(predictions)
        accuracy_6_months = ((survival_time - predictions) < 0.50).sum() / len(predictions)
        accuracy_12_months = ((survival_time - predictions) < 1.00).sum() / len(predictions)
    ...
    def test_step_onco(self, model: torch.nn.Module, batch: Tuple, step: int) -> torch.Tensor:
    ...
    def test_epoch_end(self, outputs: List) -> None:
        if dst_tools.is_main_process():
            mlflow_logging.log_metrics_list(outputs, self.global_step, prefix="TestEpoch")
    ...
    def test_onco_epoch_end(self, outputs: List) -> None:
        ...
        predictions_dict = {"sorted_ids": sorted_ids, "sorted_predictions": sorted_predictions}
        artifacts_dir = os.path.join(os.path.dirname(self.save_directory), "artifacts")
        save_path = os.path.join(artifacts_dir, "onco_predictions_epoch_{}.pickle".format(self.epoch))
    ...
    def fit(self, 
            ...
            onco_loader: data.DataLoader) -> None:
        n_training_batches, n_validation_batches = self.compute_n_batches(train_loader, val_loader)
        n_test_batches, n_onco_batches = self.compute_n_batches(test_loader, onco_loader)
...
def get_att_trainer_survival_time(logging_params: Dict,
                                  trainer_params: Dict,
                                  save_directory: str,
                                  dist_params: Dict,
    ...
    att_trainer = AttentionTrainerSurvivalTime(**logging_params,
                                                             **trainer_params,
                                                             save_directory=save_directory,
                                                             dist_params=dist_params,
                                                             optimizer=optimizer,
