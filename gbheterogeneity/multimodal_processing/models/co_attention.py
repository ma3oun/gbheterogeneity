...
class Coattention(torch.nn.Module):
    ...
    def forward(self, image: torch.Tensor, rna: torch.Tensor, register_hook: bool = False) -> torch.Tensor:
        ...
        img_qkv = self.img_qkv(image).reshape(*qkv_with_heads_shape).permute(2, 0, 3, 1, 4)
        ...
        qkv_with_heads_shape = batch_size, num_clusters, 3, self.num_heads, self.head_dim
        rna_qkv = self.rna_qkv(rna).reshape(*qkv_with_heads_shape).permute(2, 0, 3, 1, 4)
        ...
        co_rna_image = (attn_rna_image @ img_v).transpose(1, 2).reshape(batch_size, num_clusters, self.head_dim * self.num_heads)
        ...
        co_image_rna = (attn_image_rna @ rna_v).transpose(1, 2).reshape(batch_size, num_patches, self.head_dim * self.num_heads)
...
if __name__ == "__main__":
    ...
    coatt = Coattention(dim=12, img_dim=image_emb_size, rna_dim=rna_emb_size, num_heads=2)
