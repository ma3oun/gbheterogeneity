...
def train(config:dict) -> None:
    ...
    # For reproducibility
    seed = config["trainer_params"]["manual_seed"] + dist_params["rank"] if dist_params["distributed"] else 0
    ...
    train_loader = torch.utils.data.DataLoader(dataset, **dataloader_params, shuffle=False, pin_memory=True, sampler=sampler)
    ...
    if img_model_params["img_pretrained_path"]:
        state_dict = torch.load(img_model_params["img_pretrained_path"], map_location="cpu")
        img_model.load_state_dict(state_dict, strict=False)
        print("Loading TumorImage checkpoint from %s" % img_model_params["img_pretrained_path"])
    ...
    if rna_model_params["rna_pretrained_path"]:
        state_dict = torch.load(rna_model_params["rna_pretrained_path"], map_location="cpu")
        rna_model.load_state_dict(state_dict, strict=False)
        print("Loading RNA checkpoint from %s" % rna_model_params["rna_pretrained_path"])
    ...
    trainer = Trainer(**logging_params,
                    **trainer_params,
