...
class Trainer(simple_trainer.SimpleDistributedTrainer):
    ...
    def data_to_device(
        self, images: torch.Tensor, rna: Dict[str, torch.Tensor]
    ) -> Tuple:
    ...
    def compute_contrastive_loss(
        self, projections_image: torch.Tensor, projections_rna: torch.Tensor
    ) -> torch.Tensor:
    ...
    def compute_reconstruction_loss(
        ...
        loss = 0.0
        for (key_in, tensor_in), (key_out, tensor_out) in zip(
            rna_input.items(), rna_reconstruction.items()
        ):
    ...
    def compute_loss(
        self, images: torch.Tensor, rnas: Dict[str, torch.Tensor], model_outputs: Tuple
    ) -> Dict:
        (
            projections_image,
            projections_rna,
        ...
        loss_contrastive = self.compute_contrastive_loss(
            projections_image, projections_rna
        )
        loss_matching = self.compute_matching_loss(vl_outputs, vl_labels)
        loss_reconstruction = self.compute_reconstruction_loss(
            rnas, reconstructions_rna
        )
    ...
    def fit(
        ...
        model = torch.nn.parallel.DistributedDataParallel(
            model, device_ids=[self.dist_params["gpu"]], find_unused_parameters=True
        )
