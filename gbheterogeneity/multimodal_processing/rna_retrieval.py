...
def run_rna_retrieval_on_dataloader(model: torch.nn.Module, 
    ...
    projections_rna = F.normalize(model.rna_projector(representations_rna[:, 0, :]), dim=-1)
    ...
    for i, batch in enumerate(tqdm(loader,desc="Computing batch", total=loader_len)):
        ...
        projections_image = F.normalize(model.image_projector(representations_image[:, 0, :]), dim=-1)
...
def perform_rna_retrieval(model: torch.nn.Module, 
                          rna_dataloader: Iterable, 
                          train_image_loader: Iterable, 
                          val_image_loader: Iterable, 
                          device: torch.device) -> None:
    gp_accuracy_val, gl_accuracy_val = run_rna_retrieval_on_dataloader(model, rna_dataloader, val_image_loader, device)    
    mlflow_logging.log_metric(key="Accuracy_patient", value=gp_accuracy_val, step=0, prefix="Validation")
    mlflow_logging.log_metric(key="Accuracy_lineage", value=gl_accuracy_val, step=0, prefix="Validation")
    
    gp_accuracy_train, gl_accuracy_train = run_rna_retrieval_on_dataloader(model, rna_dataloader, train_image_loader, device)    
    mlflow_logging.log_metric(key="Accuracy_patient", value=gp_accuracy_train, step=0, prefix="Train")
    mlflow_logging.log_metric(key="Accuracy_lineage", value=gl_accuracy_train, step=0, prefix="Train")
    return None
...
def evaluate(config:dict) -> None:
    ...
    rna_dataset = rna_datasets.RetrievalRNASeq(oncopole_samples_directory=rna_dataset_params["oncopole_samples_directory"],
    ...
    rna_dataloader = torch.utils.data.DataLoader(rna_dataset, **rna_dataloader_params, shuffle=False)

    # Get image datasets
    # Get training set
    img_dataset_params = config["img_dataset_params"]
    dataloader_params = config["dataloader_params"]
    image_dataset = RetrievalTumorDataset(img_dataset_params=img_dataset_params,
                                                              gene_file_mapping=rna_dataset.gene_file_mapping, 
                                                              train=True)
    train_image_loader = torch.utils.data.DataLoader(image_dataset, **dataloader_params, shuffle=False)
    ...
    val_image_loader = torch.utils.data.DataLoader(val_image_dataset, **val_dataloader_params, shuffle=False)
    ...
    rna_model_params["genes_per_cluster"] = rna_dataset.genes_per_site if hasattr(rna_dataset, "genes_per_cluster") else None
    ...
    perform_rna_retrieval(model, rna_dataloader, train_image_loader, val_image_loader, device)
