...
import gbheterogeneity.utils.pytorch.device as pytorch_device
...
class SimpleTrainer(gen_trainer.GenericTrainer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.device = pytorch_device.get_device(gpu_ids=gpu_ids)
    ...
    def save_training_logs(self, image_batch: torch.Tensor, loss_dict: Dict) -> None:
    ...
    def training_epoch_end(self, outputs: List[Dict]) -> None:
        mlflow_logging.log_metrics_list(outputs, self.global_step, prefix="TrainEpoch")

    def validation_epoch_end(self, outputs: List) -> None:
        mlflow_logging.log_metrics_list(outputs, self.global_step, prefix="ValEpoch")

    def print_status(
        self, prefix: str, step: int, n_batches: int, loss_dict: Dict
    ) -> None:
    ...
    def fit(
        ...
        model = pytorch_device.model_to_device(
            model, device=self.device, gpu_ids=self.gpu_ids
        )
        ...
        for epoch_id in range(start_epoch, self.end_epoch):
            ...
            for step, batch in enumerate(train_loader):
                loss_dict = self.training_step(model, batch)
            ...
            if val_loader is not None:
                ...
                for step, batch in enumerate(val_loader):
                    loss_dict_val = self.validation_step(model, batch)
    ...
    def save_model(
        self, model: torch.nn.Module, loss_dict: Dict[str, torch.Tensor]
    ) -> None:
